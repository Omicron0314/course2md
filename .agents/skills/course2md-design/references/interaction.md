...
The user's current product decision explicitly removes the planning page and completion landing page. Resolve metadata, inspect source capabilities and build the internal plan in place after the original conversion request; do not require a second “start” click to approve an ordinary plan. Keep any useful plan details subordinate. When the note is usable, go directly to it if the person is still following that task. Otherwise update the task's completion status without taking over their navigation. Apply this to all source routes, retries and restored tasks. See the [conversion state model](states-and-motion.md#conversion-states-and-result-access); those states are not separate pages.
...
Use meaningful icons and emphasized text for the main choices and actions. Keep explanation in a quiet information panel or optional detail, with regular body typography. A title, a short primary choice and a nearby action should establish the flow before optional prose is expanded. Do not hide a consequential service choice, current setting or actionable obstacle merely to shorten the page.
