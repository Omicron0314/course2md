...
目标：关闭 [规格](UX-REDESIGN-SPEC-2026-09-07.md) 的 AT01–AT35。历史证据见 [实施与验收记录](UX-VALIDATION-2026-09-07.md)，本表追踪仍需完成的完整组合，不把单个成功分支当作整项通过。
...
| AT33 | 实际 VoiceOver：生成、错误修复、继续、逐段阅读、时间/文件动作 | 部分 | 第五轮仅焦点抽查 |
| AT34 | 所有页面纯键盘、200%、最小窗、长文本、减动效、加载/空态 | 部分 | 开关焦点、200% 阅读头部、页签、查找、导出、翻页、正文图片及截图弹层已修复并实机复测；其他页面完整键盘路径仍待验 |
...
原生文件选择面板关闭后出现自动化读取超时，进程采样显示主事件循环正常。通过明确退出并以新验证标识启动恢复连接，持久化草稿保留有效字幕；未据此声称应用崩溃。另发现法语句子合并缺少句间空格，以及工作台 Tab 到末尾按钮未自动滚入视口，继续修复和复验。
